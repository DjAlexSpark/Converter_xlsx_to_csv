...
import static io.djalexspark.converter_xlsx_to_csv.TestMain.convertXlsxToCsv;
...
public class MainController implements Initializable {
    ...
    void buttonFileFromChooserOnAction(ActionEvent event) {
        ...
        fileChooser.setTitle("Выбрать файл excel");
        fileChooser.getExtensionFilters().addAll(
                new FileChooser.ExtensionFilter("Xlsx, xlsx", "*.xlsx" ),
...
//            System.out.println(((TextField)event.getSource()).getText());
            ...
            if(Files.notExists(Path.of(((TextField) event.getSource()).getText()))){
                ((TextField) event.getSource()).setStyle("-fx-text-fill:#e85460");
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        buttonStart.setOnAction(event->{
            ...
            if (!textFieldOutputName.getText().trim().isEmpty()&&
                ...
                Path pathFrom = Path.of(textFieldWithName.getText().trim());
                Path pathTo = Path.of(textFieldDirectoryInto.getText().trim()+"\\"+textFieldOutputName.getText().trim()+".csv");
                convertXlsxToCsv(pathFrom,pathTo);
